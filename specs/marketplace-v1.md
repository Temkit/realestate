...
| **P3b (part)** | ✅ Reviews with moderation: verified reviews (only after an appointment is `completed`, via the emailed tokenized `/avis/[token]` link), admin approve/reject queue, public display on profile + cards, approved avg feeds ranking (plan → rating → name) and JSON-LD `aggregateRating`. | done |
| **P3b (rest)** | **Calendar sync (Level 3)** [needs Google/Microsoft OAuth], payments/deposits [money], SMS reminders [Twilio — cost], WhatsApp dispatch [WhatsApp Business API provider — cost], conversational search over the marketplace DB | later (each needs an owner decision) |

**Tests:** `npm test` (Vitest) runs an integration suite (`tests/`) against a real file-backed libsql engine — global-setup migrates + seeds a fresh DB, then exercises the load-bearing logic: auth tokens/signing, lead→provider matching (coverage, neighbor widening, plan ranking, cap), the Level-2 slot engine (rules − exceptions − booked − min-lead), reviews lifecycle + moderation, and provider-scoping/IDOR. No mocks. 26 tests.
